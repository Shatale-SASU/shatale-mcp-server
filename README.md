...
- **`request_purchase` is registered but BLOCKED under a sandbox key.** It appears in the sandbox column because the server advertises it there; calling it returns `sandbox_key_purchase_blocked` and steers you to `sandbox_simulate_authorization`. It is not sandbox-gated on the backend and would create real ledger state. (v0.4.0, SHAT-1488. The previous `sandbox_create_test_user`, `sandbox_decline_request`, `sandbox_reset` and `sandbox_approve_request` tools have been removed/renamed.) <!-- gone:sandbox_create_test_user --><!-- gone:sandbox_decline_request --><!-- gone:sandbox_reset --><!-- gone:sandbox_approve_request --><!-- not-a-tool:sandbox_key_purchase_blocked -->
