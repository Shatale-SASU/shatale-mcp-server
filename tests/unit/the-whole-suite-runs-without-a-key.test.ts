 ...
 *
 * /!\ AND IT DELETES tests/unit/public-suite-covers-every-deterministic-test.test.ts, WHICH WAS MY
 * OWN FIX ONE STEP EARLIER AND IS NOW WRONG RATHER THAN MERELY REDUNDANT.
 *
 * That guard asserted that every deterministic test file appears in the test:public LIST. With the
 * list gone it does not become unnecessary — it FAILS, because the thing it reads no longer exists.
 * Three of its assertions go red on a repository that is more correct than the one it was written
 * for. A guard whose subject has been removed is not dormant; it is a red build with a misleading
 * message, and the pressure is then to weaken it rather than to delete it.
 *
 * It was merged before the measurement that superseded it. Removing it here, in the change that
 * makes it wrong, is the only place the reason is legible.
 *
